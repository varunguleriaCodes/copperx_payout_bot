...
// export async function initializePusher(bot: Telegraf,ctx: Context<Update>) {
//   const chatId=ctx.chat?.id.toString();
//   const token= await ctx.getToken();
//   if(!token || !chatId) return;
//   const pusherClient = new Pusher(CONFIG?.pusher_key?.output, {
...
//   const channel = pusherClient.subscribe(`private-org-${CONFIG.organization_id.output}`);
...
//   // Listen for deposit events and send a Telegram message
//   // eslint-disable-next-line @typescript-eslint/no-explicit-any
